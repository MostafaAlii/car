...
class DriverAuthController extends Controller
{
    ...
    {
        $this->middleware('auth:captain-api', ['except' => ['checkPhone', 'login', 'register', 'login_phone', 'restPassword']]);
    }
    ...
    public function login(Request $request)
    {
        ...
        if (!$token = auth('captain-api')->attempt($validator->validated())) {
            return $this->errorResponse('Unauthorized', 422);
        }
        if (isset($request->fcm_token)) {
            Captain::where('email', $request->email)->update([
    ...
    public function login_phone(Request $request)
    {
        ...
        if (isset($request->fcm_token)) {
            Captain::where('phone', $request->phone)->update([
                'fcm_token' => $request->fcm_token,
            ]);
        }
        return $this->createNewToken($token);
    ...
    {
        return $this->createNewToken(auth('captain-api')->refresh());
    }
    ...
    protected function createNewToken($token)
    {
        return response()->json([
            'access_token' => $token,
            'token_type' => 'bearer',
            'expires_in' => auth('captain-api')->factory()->getTTL() * 80000000,
